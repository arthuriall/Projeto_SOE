...
#define BOTAO 8  // GPIO25 (WiringPi 6)
...
bool esperaClique(int pin, int debounceDelay = 50) {
    ...
    usleep(debounceDelay * 1000); // delay para estabilidade
...
int main() {
    ...
    pinMode(BOTAO, INPUT);
    pullUpDnControl(BOTAO, PUD_UP); // Botão com pull-up interno
    ...
    lcdPrint("Camera OK");
    lcdSetCursor(1, 0);
    lcdPrint("Aperte p/sair");

    std::thread t(colorThread, std::ref(cap));

    // === Espera novo clique para finalizar ===
    while (!esperaClique(BOTAO)) {
        usleep(100000);
    }
