...
// Variável estática para guardar o tempo do último clique válido
...
bool esperaClique(int pin) {
    // Verifica se o botão está pressionado (LOW) e se passou tempo suficiente desde o último clique
    if (digitalRead(pin) == LOW) {
        ...
        if (elapsed_ms > DEBOUNCE_MS) {
            ...
            while (digitalRead(pin) == LOW) {
                usleep(1000); // Pequeno atraso para não consumir CPU
            }
...
void colorThread(cv::VideoCapture& cap) {
    while (running) {
        if (leituraAtiva) {
            ...
            lcdClear();
            lcdSetCursor(0, 0);
            // Mensagem para CALIBRAÇÃO:
            // Descomente as duas linhas abaixo APENAS para calibrar
            // Comente as duas linhas abaixo quando a calibração for concluída
            // char buf_hsv[17];
            // snprintf(buf_hsv, sizeof(buf_hsv), "H:%d S:%d V:%d", h, s, v);
            // lcdPrint(buf_hsv);
            // lcdSetCursor(1, 0); // Move cursor para a linha de baixo para a cor

            // Mensagem normal de exibição da cor
...
int main() {
    ...
    // O loop de espera do botão não tem usleep aqui, pois o esperaClique já tem debounce
